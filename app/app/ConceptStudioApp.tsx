...
function reportLineItems(value: string) {
...
export default function ConceptStudioApp() {
  ...
  const reportSections = [
    ["01 성취기준과 목표", <div className="report-alignment-layout">
      <article className="report-standard-card">
        <div className="report-card-heading"><span>01</span><div><small>CURRICULUM STANDARD</small><h3>성취기준</h3></div></div>
        <p>{draft.goal.standard}</p>
      </article>
      <article className="report-goal-card">
        <div className="report-card-heading"><span>02</span><div><small>TRANSFER GOAL</small><h3>최종 학습 목표</h3></div></div>
        <p>{draft.goal.selectedGoal}</p>
      </article>
      <article className="report-alignment-check">
        <span>✓</span><div><small>ALIGNMENT CHECK</small><h3>목표 재설계 점검</h3><p>{draft.goal.transferCheck}</p></div>
      </article>
    </div>],
    ["02 개념적 렌즈·일반화·안내 질문", <div className="report-concept-layout">
      <section className="report-lens-hero">
        <div><small>CONCEPTUAL LENS</small><span>수업을 바라보는 생각의 초점</span><strong>{draft.lens.selected}</strong></div>
        <p>{draft.lens.reason}</p>
      </section>
      <section className="report-related-concepts">
        <div><small>RELATED CONCEPTS</small><h3>관련 개념</h3></div>
        <div className="report-concept-chips">{memberList(draft.concept.related).map((concept) => <span key={concept}>{concept}</span>)}</div>
      </section>
      <section className="report-generalizations">
        <div className="report-subheading"><small>TRANSFERABLE UNDERSTANDINGS</small><h3>일반화</h3></div>
        <div>
          <article><span>01</span><p>{draft.concept.generalization1}</p></article>
          <article><span>02</span><p>{draft.concept.generalization2}</p></article>
        </div>
      </section>
      <section className="report-guiding-questions">
        <div className="report-subheading"><small>GUIDING QUESTIONS</small><h3>세 종류의 안내 질문</h3></div>
        <div className="report-question-grid">
          <article className="factual"><header><span>F</span><div><b>사실적 질문</b><small>기초 지식 확인</small></div></header><div>{reportLineItems(draft.concept.factual).map((question, index) => <p key={`${question}-${index}`}><span>Q{index + 1}</span>{question}</p>)}</div></article>
          <article className="conceptual"><header><span>C</span><div><b>개념적 질문</b><small>개념 관계 탐구</small></div></header><div>{reportLineItems(draft.concept.conceptual).map((question, index) => <p key={`${question}-${index}`}><span>Q{index + 1}</span>{question}</p>)}</div></article>
          <article className="debatable"><header><span>D</span><div><b>논쟁적 질문</b><small>관점과 근거 확장</small></div></header><div>{reportLineItems(draft.concept.debatable).map((question, index) => <p key={`${question}-${index}`}><span>Q{index + 1}</span>{question}</p>)}</div></article>
        </div>
      </section>
    </div>],
